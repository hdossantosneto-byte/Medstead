  ...
  return (
    <div className="mt-6 pb-28 lg:pb-0">
      <input
        ...
        placeholder="Search IV, supplies, Non-RX…"
        value={query}
      ...
      <div className="mt-5 grid gap-6 lg:grid-cols-3">
              ...
              {rows.map((p) => {
                ...
                return (
                  <Card key={p.id} className="flex flex-col p-4">
                    ...
                    <p className="mt-2 font-semibold text-navy-900">{p.name}</p>
        ...
        <Card className="hidden h-fit p-5 lg:block">
            ...
            <ul className="mt-3 space-y-2 text-sm">
              {items.map((c) => (
                <li key={c.product.id} className="flex justify-between gap-2">
                  ...
                  <span>{money((tierFor(c.product, c.qty)?.unitPrice ?? 0) * c.qty)}</span>
                </li>
              ))}
            </ul>
          )}
          ...
          <Button className="mt-4 w-full" disabled={!items.length || busy} onClick={submit}>
      ...
      <div className="fixed inset-x-0 bottom-0 z-20 border-t border-navy-900/10 bg-white/95 p-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] lg:hidden">
        <div className="mx-auto flex max-w-6xl items-center gap-3">
          ...
          <Button className="min-h-tap flex-1" disabled={!items.length || busy} onClick={submit}>
            {busy ? "Submitting…" : "Place order"}
          </Button>
        </div>
